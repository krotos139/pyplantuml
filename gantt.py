    ...
    def search_for_task_conflicts(self, all_tasks=False):
        ...
        for d in ke:
            if len(affected_days[d]) > 1:
                overcharged_days[d] = affected_days[d]
                __LOG__.warning('** Resource "{2}" has more than one task on day {0} / {1}'.format(d, affected_days[d], self.name.encode("utf8")))
...
class Task(object):
    ...
    def start_date(self):
        ...
        __LOG__.debug('** Task::start_date ({0})'.format(self.name.encode("utf8")))
        if self.start is not None:
            ...
            if self.depends_of is None:
                ...
                if start > self.start:
                    __LOG__.warning('** Due to vacations, Task "{0}", will not start on date {1} but {2}'.format(self.fullname.encode("utf8"), self.start, start))
                ...
                if prev_task_end > self.start:
                    __LOG__.warning('** Due to dependencies, Task "{0}", will not start on date {1} but {2}'.format(self.fullname.encode("utf8"), self.start, prev_task_end))
    ...
    def end_date(self):
        ...
        __LOG__.debug('** Task::end_date ({0})'.format(self.name.encode("utf8")))
        ...
        if self.duration is None or self.start is None and self.stop is not None:
            ...
            if real_end <= self.start_date():
                ...
                __LOG__.warning('** task "{0}" will not be finished on time : end_date is changed from {1} to {2}'.format(self.fullname.encode("utf8"), self.stop, self.cache_end_date))
                return self.cache_end_date
    ...
    def svg(self, prev_y=0, start=None, end=None, color=None, level=None, scale=DRAW_WITH_DAILY_SCALE, title_align_on_left=False):
        ...
        svg = svgwrite.container.Group(id=self.name.replace(' ', '_').encode("utf8"))
        ...
        if self.resources is not None:
            t = " / ".join(["{0}".format(r.name.encode("utf8")) for r in self.resources])
    ...
    def csv(self, csv=None):
        ...
        csv_text = '"{0}";"{1}";{2};{3};{4};"{5}";\r\n'.format(
            self.state.replace('"', '\\"'),
            self.fullname.replace('"', '\\"').encode("utf8"),
            self.start_date(),
            self.end_date(),
            self.duration,
            resources.replace('"', '\\"').encode("utf8")
            )
        return csv_text
...
class Project(object):
    ...
    def make_svg_for_resources(self, filename, today=None, start=None, end=None, resources=None, one_line_for_tasks=False, filter='', scale=DRAW_WITH_DAILY_SCALE):
        ...
        for r in resources:
            ...
            ress.add(svgwrite.text.Text('{0}'.format(r.fullname.encode("utf8")), insert=(3*mm, (nline*10+7)*mm), fill=_font_attributes()['fill'], stroke=_font_attributes()['stroke'], stroke_width=_font_attributes()['stroke_width'], font_family=_font_attributes()['font_family'], font_size=15+3))
            #ldwg.add(ress)
    ...
    def svg(self, prev_y=0, start=None, end=None, color=None, level=0, scale=DRAW_WITH_DAILY_SCALE, title_align_on_left=False):
        ...
        if self.name != "":
            ...
            if ((self.start_date() >= start and self.end_date() <= end) 
                ...
                fprj.add(svgwrite.text.Text('{0}'.format(self.name.encode("utf8")), insert=((6*level+3)*mm, ((prev_y)*10+7)*mm), fill=_font_attributes()['fill'], stroke=_font_attributes()['stroke'], stroke_width=_font_attributes()['stroke_width'], font_family=_font_attributes()['font_family'], font_size=15+3))
